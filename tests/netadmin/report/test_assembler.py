...
def seeded(repo: Repository) -> dict[str, int]:
    ...
    rogue = repo.insert_issue(
        fingerprint="fp-rogue",
        detector_key="wifi.rogue_ap",
        severity="p3",
        state="active",
        first_seen_ts=BUCKET,
        last_seen_ts=SEEN,
        title="Rogue AP on channel 6",
        entity_id=r_core,
    )
    ...
    ids.update(mesh=mesh, cov=cov, flaky=flaky, rogue=rogue, chan=chan)
...
def test_environmental_noise_collapses_to_one_finding(seeded, repo) -> None:
    ...
    assert len(env) == 1
    assert set(env[0]["source_issue_ids"]) == {seeded["rogue"], seeded["chan"]}
    # No per-detector rogue/channel findings leak through.
    assert not any(
        f["detector_key"] in ("wifi.rogue_ap", "wifi.channel_plan") for f in doc["findings"]
    )
