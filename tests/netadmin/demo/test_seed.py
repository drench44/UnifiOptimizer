...
def test_issue_counts_by_state_and_severity(demo_db) -> None:
    ...
    # + airtime + client.flaky) + a standalone rogue_ap = 16.
...
def test_issue_spread_and_lifecycle(demo_db) -> None:
    ...
    try:
        ...
        for key in (
            ...
            # The correlation showcase: the mesh cluster + a standalone rogue.
            "wifi.mesh_uplink",
            "net.coverage_hole",
            "client.flaky",
            "wifi.rogue_ap",
...
def test_correlation_incidents_grouped(demo_db) -> None:
    ...
    try:
        ...
        # The rogue AP could not be attributed to any root -> incident-of-one.
        rogue = by_root_key["wifi.rogue_ap"]
        assert len(repo.list_incident_members(int(rogue["id"]))) == 1
    finally:
        repo.close()
