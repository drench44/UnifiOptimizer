...
class FixService:
    ...
    async def build_plan(self, issue_id: int) -> FixPlan:
        """Reconstruct the finding, read the device read-only, and plan the fix.

        For an advisory detector (physical fix) the device is never read: there is
        nothing to render, so we do not touch the controller at all.
        """
        ...
        if finding.detector_key not in PHYSICAL_REFUSAL_KEYS and self._reader is not None:
            device = await self._reader.read_device(device_mac_of(finding.entity.native_id))
        return plan_fix(finding, device=device, issue_id=issue_id)
    ...
    async def apply(self, issue_id: int, *, confirm_token: str) -> ApplyResult:
        ...
        if result.applied and plan.issue_id is not None:
            self._verifier.arm(
                plan.issue_id,
                self._now_fn(),
                detail={
