...
* :class:`SfpDegradedDetector` (``wired.sfp_degraded``) — SFP rx-power near the
  sensitivity floor or drifting down, or an rx/tx fault flag.
...
class SfpDegradedDetector:
    """``wired.sfp_degraded`` — SFP rx-power near the floor / drifting, or a fault -> P2."""
    ...
    def evaluate(self, ctx: Any) -> EvalResult:
        ...
        for port in _ports(ctx):
            ...
            if rx is None and not rxfault and not txfault:
