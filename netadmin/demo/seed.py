...
class _Seeder:
    ...
    def build_issues(self) -> None:
        with self.repo.transaction():
            ...
            # A standalone rogue neighbour AP, so the demo shows a rogue_ap issue
            # rendering like any other (its own incident-of-one).
            self._issue_rogue_ap()
    ...
    def _issue_rogue_ap(self) -> None:
        """A strong, persistent neighbour AP co-channel with our Kitchen 2.4G radio.

        A standalone finding (its own incident-of-one) so the demo shows a
        ``wifi.rogue_ap`` issue rendering like any other. Fabricated BSSID (``02:``
        space) and a generic SSID — never a real neighbour's network name.
        """
        radio = self.aps["Kitchen"]["radios"]["ng"]
        rogue_bssid = "02:00:5e:99:14:22"
        first = self.now - 4 * DAY
        iid = self._insert(
            ...
            severity=Severity.P3,
            state=IssueState.ACTIVE,
            title="Rogue/neighbor AP NEIGHBOR-2G4 on our channel 11",
            first_seen=first,
            last_seen=self.now - HOUR,
            evidence={
                "bssid": rogue_bssid,
