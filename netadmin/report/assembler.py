...
# The two detectors whose findings are RF-environmental noise: rogue/neighbour APs
# and channel-plan contention. They are pulled out of the incident grouping and
# summarised as ONE environmental finding, never dozens of per-BSSID alarms
# (docs/REPORT_SPEC.md anti-patterns).
ENVIRONMENTAL_KEYS = frozenset({"wifi.rogue_ap", "wifi.channel_plan"})
...
def _environmental_finding(
    ...
    """Collapse rogue/channel-plan issues + neighbour density into ONE finding."""
    rogue = [i for i in env_issues if i["detector_key"] == "wifi.rogue_ap"]
    ...
    if rogue:
        rec_parts.append(finding_guidance("wifi.rogue_ap").recommendation)
    ...
    channels = len(neighbor_density["by_channel"])
    observation = (
        f"{_plural(len(rogue), 'strong neighbour AP')} and "
    ...
    finding = Finding(
        ...
        observation=observation,
        evidence={
            "rogue_ap_issue_count": len(rogue),
