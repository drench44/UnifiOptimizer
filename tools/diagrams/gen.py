...
def pl(pts, w=2.3, col=INK, op=0.92, dash=None, closed=False):
    ...
    return (f'<polyline points="{polystr(p)}" fill="none" stroke="{col}" '
...
def stroke(x1, y1, x2, y2, col=INK, amp=1.7, key=0, double=True, dash=None, w=2.3):
    ...
    if double and dash is None:
        s += pl(wobble(x1, y1, x2, y2, amp=amp * 1.35, key=(key, "b")), w=w * 0.55, col=col, op=0.38)
    return s
...
def box(x, y, w, h, key=0, col=INK, fill=None, amp=1.8, wmain=2.3, fillop=1.0):
    ...
    out = []
    if fill:
        out.append(f'<polygon points="{polystr(pts)}" fill="{fill}" opacity="{fillop}" stroke="none"/>')
...
def arrow(x1, y1, x2, y2, col=INK, key=0, amp=1.7, dash=None, head=13, w=2.3):
    out = [stroke(x1, y1, x2, y2, col=col, amp=amp, key=(key, "sh"), double=(dash is None), dash=dash, w=w)]
...
def curve_arrow(pts, col=INK, key=0, amp=1.5, dash=None, head=13, w=2.3):
    ...
    for i in range(len(pts) - 1):
        seg.append(pl(wobble(*pts[i], *pts[i + 1], amp=amp, key=(key, "c", i)),
                      w=w, col=col, op=0.9, dash=dash))
    x1, y1 = pts[-2]
    x2, y2 = pts[-1]
    ang = math.atan2(y2 - y1, x2 - x1)
    for s in (1, -1):
...
def text(x, y, s, size=20, col=INK, anchor="middle", spacing=None):
    ...
    return (f'<text x="{x:.1f}" y="{y:.1f}" text-anchor="{anchor}"{ls} '
...
def lines(x, y, rows, size=19, col=INK, anchor="middle", lh=None):
    lh = lh or size * 1.22
    return "".join(text(x, y + i * lh, r, size=size, col=col, anchor=anchor) for i, r in enumerate(rows))
...
def clock(cx, cy, r, key, col=INK):
    out = [pl(ellipse_pts(cx, cy, r, r, (key, "clk"), amp=0.9, n=30), w=2.0, col=col, op=0.9, closed=True)]
    out.append(stroke(cx, cy, cx, cy - r * 0.62, col=col, amp=0.6, key=(key, "h1"), double=False, w=2.0))
    out.append(stroke(cx, cy, cx + r * 0.5, cy + r * 0.12, col=col, amp=0.6, key=(key, "h2"), double=False, w=2.0))
    return "".join(out)


def check(cx, cy, s, key, col=ACCENT, w=3.0):
    return (stroke(cx - s, cy, cx - s * 0.25, cy + s * 0.7, col=col, amp=0.7, key=(key, "k1"), double=False, w=w)
            + stroke(cx - s * 0.25, cy + s * 0.7, cx + s, cy - s * 0.7, col=col, amp=0.7, key=(key, "k2"), double=False, w=w))
...
def cylinder(cx, top, w, h, key, fill=ACCENT_FILL, col=INK):
    ...
    for frac in (0.40, 0.70):
        yy = top + h * frac
        arc = ellipse_pts(cx, yy, rx, ry, (key, "sh", round(frac * 100)), amp=0.8, n=22, a0=0.0, a1=math.pi)
...
def header(w, title, subtitle, tag):
    ...
    out.append(box(w - 168, 44, 118, 40, key=("tag", tag), col=INK2, fill=BOX_FILL, amp=1.6, wmain=1.6))
...
def diagram_watches():
    W, H = 1860, 1080
    b = [header(W, "How UnifiOptimizer watches", "collect, remember, detect, track, then tell you", "fig 1")]
    ...
    b.append(lines(ax + aw / 2, ay + 96, ["REST API", "stat/device, sta,", "health   (every 60s)"], size=17, col=INK2))
    b.append(box(ax + 20, ay + 176, aw - 40, 96, key="A2", col=INK, fill=PAPER, amp=1.6, wmain=1.7))
    b.append(lines(ax + aw / 2, ay + 210, ["Event WebSocket", "EVT_* keys,", "live"], size=17, col=INK2))
    ...
    b.append(lines(cx, cyl_top + 176, ["raw 30d, then hourly 18mo,", "then daily  -  forever"], size=17, col=INK2))
    b.append(lines(cx, cyl_top + 250, ["counters kept as rates;", "gaps recorded, never zeroed"], size=17, col=INK2))
    # "it remembers" banner
    b.append(box(cx - 118, cyl_top - 66, 236, 46, key="rem", col=ACCENT, fill=ACCENT_FILL, amp=1.7, wmain=2.4))
    ...
    b.append(box(dx + 16, 356, dw - 32, 52, key="D1c", col=ACCENT, fill=ACCENT_FILL, amp=1.5, wmain=2.0))
    b.append(lines(dx + dw / 2, 380, ["confounder checks:", "the false alarms it ruled out"], size=15, col=ACCENT, lh=17))

    b.append(box(dx, 452, dw, 128, key="D2", col=INK, fill=BOX_FILL, amp=1.9))
    b.append(text(dx + dw / 2, 490, "SLE health minutes", size=23))
    b.append(lines(dx + dw / 2, 522, ["each active client-minute", "judged pass or fail"], size=17, col=INK2))
    ...
    for i, s in enumerate(states):
        ...
        if i < len(states) - 1:
            b.append(arrow(ex + ew / 2, yy + 12, ex + ew / 2, yy + 28, key=("ea", i), amp=0.8, head=8, w=1.7, col=INK2))
    ...
    ly = 940
    b.append(curve_arrow([(ex + ew / 2, 550), (ex + ew / 2, ly), (ax + aw / 2, ly), (ax + aw / 2, ay + ah + 8)],
                         key="loop", amp=1.6, dash="9 8", col=ACCENT))
    b.append(box(cx - 300, ly - 34, 600, 62, key="apr", col=ACCENT, fill=ACCENT_FILL, amp=1.8, wmain=2.4))
    b.append(check(cx - 262, ly - 3, 12, key="aprk"))
    b.append(text(cx + 14, ly + 6, "a fix is applied only when YOU approve it", size=23, col=ACCENT))
...
def diagram_issue():
    W, H = 1820, 1040
    b = [header(W, "Life of an issue", "one issue per fingerprint, not a new alert every poll", "fig 2")]
    ...
    for i, yy in enumerate((ymid - 92, ymid, ymid + 92)):
        b.append(box(fnx - 62, yy - 26, 124, 52, key=("find", i), col=INK2, fill=PAPER, amp=1.7, wmain=1.7))
        b.append(text(fnx, yy + 6, "finding", size=18, col=INK2))
        b.append(arrow(fnx + 64, yy, fp - nodew / 2 - 8, ymid + (yy - ymid) * 0.18, key=("ff", i), amp=1.4, col=INK2, w=1.9))
    ...
    b.append(box(fp - 150, ymid + 70, 300, 78, key="fpn", col=ACCENT, fill=ACCENT_FILL, amp=1.7, wmain=2.2))
    b.append(lines(fp, ymid + 100, ["many findings collapse", "into ONE issue"], size=18, col=ACCENT, lh=22))
    b.append(text(fp, ymid + 176, "sha1(detector + device + dims)", size=15, col=INK2))

    b.append(lines(pe, ymid + 92, ["seen -- but wait.", "hold ~3 polls first"], size=17, col=INK2, lh=21))
    b.append(lines(rs, ymid + 92, ["looks clear:", "count 6 clean polls"], size=17, col=INK2, lh=21))
    ...
    b.append(box(sbx - 118, sby - 37, 236, 74, key="acd", col=ACCENT, fill=ACCENT_FILL, amp=1.7, wmain=2.3))
    ...
    b.append(arrow(sbx + 40, sby + 37, ac - 55, ymid - nodeh / 2 - 6, key="acdA", amp=1.2, head=10, w=1.9, col=ACCENT))

    # resolved check
    b.append(check(rd + nodew / 2 - 26, ymid - nodeh / 2 + 20, 10, key="rdk"))

    # snap-back: fire during resolving -> active
    b.append(curve_arrow([(rs, ymid + nodeh / 2 + 6), (rs - 90, ymid + 128), (ac, ymid + 128), (ac, ymid + nodeh / 2 + 6)],
                         key="snap", amp=1.4, dash="7 7", col=INK2, w=1.9))
    b.append(text((ac + rs) / 2, ymid + 150, "a fire during resolving snaps back to active", size=16, col=INK2))

    # reopen loop (resolved -> active), routed high to clear the callout
    topy = ymid - 260
    b.append(curve_arrow([(rd, ymid - nodeh / 2 - 6), (rd, topy), (ac + 55, topy), (ac + 55, ymid - nodeh / 2 - 6)],
                         key="reopen", amp=1.4, dash="9 8", col=ACCENT, w=2.1))
    b.append(text((ac + rd) / 2 + 30, topy - 12, "refires within a day: reopen the SAME row (not a new issue)", size=17, col=ACCENT))

    # ---- inhibition: a bigger fault mutes the small ones ---------------
    iy = 830
    b.append(underline(150, 470, iy - 30, key="inhu"))
    b.append(text(150, iy - 36, "a bigger fault mutes the small ones", size=24, col=INK, anchor="start"))
    ...
    for i, lab in enumerate(["port A: rx errors", "port B: flapping"]):
        yy = iy - 6 + i * 56
        b.append(box(700, yy, 260, 46, key=("sm", i), col=INK2, fill=IDLE_FILL, amp=1.6, wmain=1.6, fillop=0.7))
        b.append(text(830, yy + 30, lab, size=18, col=INK2))
        b.append(arrow(392, iy + 42, 694, yy + 22, key=("inh", i), amp=1.4, dash="8 7", col=ACCENT, w=2.0))
    b.append(text(1120, iy + 44, "muted while the switch is down", size=19, col=INK2, anchor="start"))
    b.append(text(1120, iy + 72, "(absence of evidence is not a fix)", size=16, col=INK2, anchor="start"))
...
def diagram_sle():
    W, H = 1780, 812
    b = [header(W, "Health as user-minutes", "each active client-minute is judged pass or fail", "fig 3")]
    ...
    # column header (time)
    b.append(text(gx + cols * (cw + gap) / 2, gy - 26, "one hour, in 5-minute buckets  (time ->)", size=18, col=INK2))
    b.append(arrow(gx, gy - 12, gx + cols * (cw + gap) - gap, gy - 12, key="taxis", amp=1.0, head=10, w=1.6, col=INK2))

    def cell(x, y, kind, key):
        fill = {"p": OK_FILL, "f": ACCENT_FILL, "i": IDLE_FILL}[kind]
        col = ACCENT if kind == "f" else INK
        out = [box(x, y, cw, ch, key=("cell", key), col=col, fill=fill, amp=1.5, wmain=2.0,
        ...
        if kind == "p":
            ...
            # a small "x" cross
            out.append(stroke(cx - 11, cy - 11, cx + 11, cy + 11, col=ACCENT, amp=0.6, key=("x1", key), double=False, w=2.8))
            out.append(stroke(cx + 11, cy - 11, cx - 11, cy + 11, col=ACCENT, amp=0.6, key=("x2", key), double=False, w=2.8))
        else:
            out.append(stroke(x + 14, y + ch - 14, x + cw - 14, y + 14, col=INK2, amp=0.6, key=("id", key), double=False, w=1.4))
        return "".join(out)

    fail_cell_xy = None
    for r, (label, states) in enumerate(rows):
        y = gy + r * (ch + gap)
        b.append(text(gx - 20, y + ch / 2 + 6, label, size=21, anchor="end",
    ...
    if fail_cell_xy:
        ...
        b.append(curve_arrow([(fxp + 4, fyp), (fxp + 120, fyp + 34), (cbx - 40, cby + cbh - 20), (cbx - 2, cby + cbh - 46)],
                             key="cocall", amp=1.3, col=ACCENT, w=2.1))
        b.append(box(cbx, cby, cbw, cbh, key="cobox", col=ACCENT, fill=ACCENT_FILL, amp=1.8, wmain=2.3))
        b.append(text(cbx + cbw / 2, cby + 38, "one failed minute", size=22, col=ACCENT))
        b.append(lines(cbx + cbw / 2, cby + 76,
                       ["pinned to ONE cause:  weak_signal", "on ONE device:  AP-2"], size=18, col=INK, lh=26))
        b.append(text(cbx + cbw / 2, cby + 138, "(exclusive attribution)", size=15, col=INK2))

    # idle emphasis
    iy = gy + 2 * (ch + gap) + ch / 2
    b.append(text(gx + cols * (cw + gap) + 30, iy - 10, "idle, and yes the signal is bad", size=18, col=INK2, anchor="start"))
    b.append(text(gx + cols * (cw + gap) + 30, iy + 16, "-> still 0 failed minutes", size=19, col=ACCENT, anchor="start"))
...
def main():
    ...
    for name, fn in figs:
        ...
        print(f"wrote {svg_path.relative_to(ROOT)}" + (f" + {png_path.relative_to(ROOT)}" if ok else " (SVG only)"))
    if not rastered:
